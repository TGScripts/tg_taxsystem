...
ESX.RegisterServerCallback('tg_taxsystem:getpropertycount', function (source, cb)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    MySQL.Async.fetchAll('SELECT COUNT(*) AS COUNTER FROM datastore_data WHERE owner = @owner AND name LIKE "property";', 
    {
        ['@owner'] = xPlayer.identifier
    }, 
    function (result)
